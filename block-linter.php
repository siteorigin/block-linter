...
class BlockLinter {
	 ...
	 * @var array
	 */
	private $post_validation_callbacks = array();
	 ...
	 * @var bool
	 */
	private static $library_mode = false;
	 ...
	 */
	public static function set_library_mode( $enabled = true ) {
	 ...
	 */
	public function add_custom_validator( $validator ) {
	 ...
	 */
	public function add_post_validation_callback( $callback ) {
		if ( is_callable( $callback ) ) {
			$this->post_validation_callbacks[] = $callback;
		}
	}
	...
	public function lint_file( $filepath ) {
	...
	public function lint( $content, $source = 'input' ) {
		...
		$this->validate_block_count( $blocks );
		$this->validate_blocks( $blocks, 0, array() );
		$this->check_for_unclosed_blocks( $content );
		$this->check_for_orphaned_closers( $content );

		// Check for malformed JSON attributes
		if ( $this->config['check_malformed_json'] ) {
			$this->check_malformed_json( $content );
		}
		...
		foreach ( $this->post_validation_callbacks as $callback ) {
			try {
				$result = call_user_func( $callback, $modified_content, $this->errors, $this->warnings, $this );
				if ( is_string( $result ) ) {
	 ...
	 */
	public function get_modified_content( $content ) {
		$modified_content = $content;
		foreach ( $this->post_validation_callbacks as $callback ) {
			try {
				$result = call_user_func( $callback, $modified_content, $this->errors, $this->warnings, $this );
	...
	private function validate_blocks( $blocks, $depth, $parent_blocks = array() ) {
		foreach ( $blocks as $block ) {
			if ( ! empty( $block['blockName'] ) ) {
				$this->validate_block( $block, $depth, $parent_blocks );
			}
			...
			if ( ! empty( $block['innerBlocks'] ) ) {
				...
				$this->validate_blocks( $block['innerBlocks'], $depth + 1, $new_parent_blocks );
			}
		}
	}

	private function validate_block( $block, $depth, $parent_blocks = array() ) {
		// Check nesting depth
		...
		// Validate block name
		$this->validate_block_name( $block['blockName'] );

		// Validate attributes
		if ( ! empty( $block['attrs'] ) ) {
			$this->validate_attributes( $block );
		}

		// Check for empty blocks
		if ( $this->config['check_empty_blocks'] ) {
			$this->check_empty_block( $block );
		}

		// Validate parent-child relationships
		if ( $this->config['validate_parent_child_relationships'] ) {
			$this->validate_parent_child_relationship( $block['blockName'], $parent_blocks );
		}

		// Validate specific block types
		$this->validate_specific_block_type( $block );
	}

	private function validate_block_name( $blockName ) {
	...
	private function validate_attributes( $block ) {
		...
		$this->validate_block_specific_attributes( $block );
	}

	private function validate_block_specific_attributes( $block ) {
	...
	private function check_empty_block( $block ) {
	...
	private function validate_specific_block_type( $block ) {
		switch ( $block['blockName'] ) {
			case 'core/column':
	...
	private function validate_block_count( $blocks ) {
	...
	private function check_for_unclosed_blocks( $content ) {
	...
	private function check_for_orphaned_closers( $content ) {
	...
	public function format_results( $verbose = false ) {
		$output = array();
	...
	private function validate_parent_child_relationship( $blockName, $parentBlocks ) {
	...
	private function check_malformed_json( $content ) {
...
// CLI Interface
if ( php_sapi_name() === 'cli' && ! defined( 'BLOCK_LINTER_LIBRARY_MODE' ) ) {
...
HELP;
	...
	if ( isset( $options['f'] ) || isset( $options['file'] ) ) {
		...
		$result = $linter->lint_file( $file );

		echo "Linting: $file\n";
		echo $linter->format_results( $verbose );
		echo "\n\n";
